document.addEventListener("DOMContentLoaded", () => {
  ...
  const buttons = document.querySelectorAll(
    ".hero-btn, .saiba-mais-link, .delivery-card",
  );
  ...
  buttons.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      console.log(
        `Botão ou Card "${btn.textContent.trim() || btn.querySelector(".card-title")?.textContent.trim()}" clicado (apenas decorativo)`,
      );

      // Feedback sutil para simular funcionamento
      if (btn.classList.contains("hero-btn")) {
        window.location.href = "src/auth/cadastro.html";
      }
    });
  });
